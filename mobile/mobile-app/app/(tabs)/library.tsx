...
export default function LibraryScreen() {
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "library"), { title, imageURL, available: true, borrower: null, createdAt: serverTimestamp(), owner: auth.currentUser.email });
  ...
  const filteredItems = libraryItems.filter(item => item.title.toLowerCase().includes(searchQuery.toLowerCase()));
            ...
            <View style={styles.itemsList}>
              {filteredItems.map(item => (
                <View key={item.id} style={styles.itemRow}>
                  ...
                  <TouchableOpacity style={styles.borrowBtn}><Text style={styles.borrowBtnText}>استعر</Text></TouchableOpacity>
                </View>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  searchInput: {
    borderWidth: 3,           // تم تعديل الحد ليصبح سميك
    borderColor: '#1a3a2a',  
    marginHorizontal: 16,
    marginBottom: 10,
    padding: 10,
